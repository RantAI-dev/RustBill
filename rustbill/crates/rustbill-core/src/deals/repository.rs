...
use crate::deals::schema::{CreateDealRequest, UpdateDealRequest};
...
pub trait DealsRepository: Send + Sync {
    ...
    async fn create_deal(&self, req: CreateDealRequest) -> Result<Deal>;
    async fn update_deal(&self, id: &str, req: UpdateDealRequest) -> Result<Deal>;
    async fn delete_deal(&self, id: &str) -> Result<()>;
}

pub struct PgDealsRepository<'a> {
    pool: &'a PgPool,
}

impl<'a> PgDealsRepository<'a> {
...
#[async_trait]
impl DealsRepository for PgDealsRepository<'_> {
    async fn list_deals(
        &self,
        product_type: Option<&str>,
        deal_type: Option<&str>,
        ...
        if product_type == ProductType::Licensed {
            if let Some(ref lk) = row.license_key {
                ...
                let expires_at = chrono::Utc::now()
                    ...
                    .ok_or_else(|| {
                        BillingError::bad_request("failed to compute license expiration date")
                    })?
                    .format("%Y-%m-%d")
                    .to_string();
                ...
                if let Ok(Some(_keypair)) = crate::licenses::get_keypair(self.pool).await {
                    let _ = crate::licenses::sign_license_by_key(self.pool, lk).await;
                }
            }
        }
    ...
    async fn update_deal(&self, id: &str, req: UpdateDealRequest) -> Result<Deal> {
        let existing = <Self as DealsRepository>::get_deal(self, id).await?;
        ...
        {
            emit_deal_reversal_and_replacement(self.pool, &existing, &row, "deal_update").await?;
        }
    ...
    async fn delete_deal(&self, id: &str) -> Result<()> {
        ...
        emit_deal_reversal(self.pool, &existing, "deal_delete", None).await?;
...
async fn create_invoice_from_deal(
    ...
    let due_at = chrono::Utc::now()
        ...
        .ok_or_else(|| BillingError::bad_request("failed to compute invoice due date"))?
        .format("%Y-%m-%d")
        .to_string();
