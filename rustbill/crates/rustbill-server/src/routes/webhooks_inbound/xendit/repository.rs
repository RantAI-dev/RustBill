...
use rustbill_core::db::models::Invoice;
use rustbill_core::error::BillingError;
use rustbill_core::notifications::email::EmailSender;
use sqlx::PgPool;

#[async_trait]
pub trait XenditWebhookRepository: Send + Sync {
...
#[derive(Clone)]
pub struct SqlxXenditWebhookRepository {
    pool: PgPool,
}

impl SqlxXenditWebhookRepository {
...
#[async_trait]
impl XenditWebhookRepository for SqlxXenditWebhookRepository {
    async fn record_event(
        &self,
        event_type: &str,
        resource_id: Option<&str>,
        ...
        sqlx::query(
            r#"INSERT INTO billing_events (id, event_type, resource_type, resource_id, data, created_at)
               VALUES (gen_random_uuid()::text, $1::billing_event_type, 'xendit', COALESCE($2, ''), $3, now())"#,
        )
        .bind(event_type)
        .bind(resource_id)
        .bind(data)
