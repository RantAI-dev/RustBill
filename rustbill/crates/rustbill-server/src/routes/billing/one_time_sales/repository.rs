use super::schema::{CreateOneTimeSaleRequest, OneTimeSaleItemInput, UpdateOneTimeSaleRequest};
...
#[async_trait]
pub trait OneTimeSalesRepository: Send + Sync {
    ...
    async fn delete_scoped(&self, id: &str, customer_id: &str) -> Result<u64, BillingError>;
}

#[derive(Clone)]
pub struct SqlxOneTimeSalesRepository {
    pool: PgPool,
}

impl SqlxOneTimeSalesRepository {
...
#[async_trait]
impl OneTimeSalesRepository for SqlxOneTimeSalesRepository {
    async fn list_admin(&self) -> Result<Vec<serde_json::Value>, BillingError> {
    ...
    async fn get_admin(&self, id: &str) -> Result<serde_json::Value, BillingError> {
        ...
        .await
        .map_err(BillingError::from)?
        .ok_or_else(|| BillingError::not_found("one_time_sale", id))
    }
    ...
    async fn create_admin(
        ...
        let row = sqlx::query_scalar::<_, serde_json::Value>(
            r#"INSERT INTO invoices (id, invoice_number, customer_id, subscription_id, status, currency, subtotal, tax, total, due_at, issued_at, notes, created_at, updated_at)
               VALUES (gen_random_uuid()::text, $1, $2, NULL, $3::invoice_status, $4, $5, $6, $7, $8::timestamp, $9::timestamp, $10, now(), now())
    ...
    async fn emit_created_event(
        ...
        let tax_dec =
            Decimal::from_str(&body.normalized_tax().to_string()).unwrap_or(Decimal::ZERO);
    ...
    async fn delete_scoped(&self, id: &str, customer_id: &str) -> Result<u64, BillingError> {
        let result = sqlx::query(
            r#"UPDATE invoices
               SET status = 'void', deleted_at = now(), updated_at = now()
               WHERE id = $1
                 AND subscription_id IS NULL
                 AND deleted_at IS NULL
                 AND customer_id = $2"#,
