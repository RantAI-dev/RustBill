...
async fn analytics_overview_returns_expected_fields(pool: PgPool) {
    ...
    if status == axum::http::StatusCode::OK {
        ...
        assert!(body.get("totalCustomers").is_some(), "missing totalCustomers");
        assert!(body.get("activeSubscriptions").is_some(), "missing activeSubscriptions");
        assert!(body.get("mrr").is_some(), "missing mrr");
        assert!(body.get("totalRevenue").is_some(), "missing totalRevenue");
        assert!(body.get("activeLicenses").is_some(), "missing activeLicenses");
    } else {
...
async fn analytics_forecasting_returns_expected_fields(pool: PgPool) {
    ...
    assert!(body["forecastData"].is_array(), "forecastData should be an array");
    ...
    let kpis = &body["kpis"];
    assert!(kpis.get("currentQuarterForecast").is_some(), "missing kpis.currentQuarterForecast");
    assert!(kpis.get("quarterTarget").is_some(), "missing kpis.quarterTarget");
    assert!(kpis.get("dealCoverage").is_some(), "missing kpis.dealCoverage");
    assert!(kpis.get("atRiskRevenue").is_some(), "missing kpis.atRiskRevenue");
}
...
async fn analytics_reports_returns_expected_fields(pool: PgPool) {
    ...
    let body: serde_json::Value = resp.json();
    assert!(body.get("conversionData").is_some(), "missing conversionData");
    ...
    assert!(body["conversionData"].is_array(), "conversionData should be an array");
    assert!(body["sourceData"].is_array(), "sourceData should be an array");
