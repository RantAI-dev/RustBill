    ...
    match rule {
        ...
        None => {
            if let Some(external) =
                resolve_external_tax(pool, country, region, product_category, subtotal).await?
            {
                return Ok(external);
            }
            Ok(TaxResult::zero())
...
async fn resolve_external_tax(
    ...
    let provider = get_setting(pool, "external_tax_provider").await?;
    if provider.is_empty() {
        return Ok(None);
    }

    let external = match provider.to_lowercase().as_str() {
        "taxjar" => fetch_taxjar_tax(pool, country, region, subtotal).await,
        "stripe" => fetch_stripe_tax(pool, country, region, subtotal).await,
        _ => Ok(None),
    };
    ...
    cache_external_rule(pool, country, region, product_category, &external).await?;
